...
export function DiscoverRows() {
  ...
  return (
    <div className="space-y-6 px-[5vw] py-4">
      {/* Ranking Filmes */}
      <section className="space-y-3">
        <h2 className="flex items-center gap-2 text-lg font-black uppercase tracking-[0.2em] text-white">
          <span className="text-vexia-purple text-base">01</span> 10 Melhores Filmes
        </h2>
        <div className="vexia-fade-edges vexia-smooth-scroll flex gap-4 overflow-x-auto pb-2 vexia-scroll no-scrollbar">
          {topMovies.map((item, index) => (
            <Link
              key={item.id}
              to="/detalhes/$id"
              params={{ id: item.id }}
              className="group relative flex w-[160px] shrink-0 items-end gap-1.5 outline-none"
            >
      ...
      <section className="space-y-3">
        <h2 className="flex items-center gap-2 text-lg font-black uppercase tracking-[0.2em] text-white">
          <span className="text-vexia-cyan text-base">02</span> 10 Melhores Séries
        </h2>
        <div className="vexia-fade-edges vexia-smooth-scroll flex gap-4 overflow-x-auto pb-2 vexia-scroll no-scrollbar">
